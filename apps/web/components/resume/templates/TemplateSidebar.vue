    ...
    <div class="grid grid-cols-[1fr_160px] gap-10 min-h-full">
      <main>
        ...
        <section v-if="hasSummary" class="mb-8">
          <h2 class="text-[7pt] uppercase tracking-[0.2em] font-bold text-[#94a3b8] mb-3">Profil</h2>
        ...
        <section v-if="hasExperiences" class="mb-8">
          <h2 class="text-[7pt] uppercase tracking-[0.2em] font-bold text-[#94a3b8] mb-4">Expérience</h2>
          <div
        ...
        <section v-if="hasEducations">
          <h2 class="text-[7pt] uppercase tracking-[0.2em] font-bold text-[#94a3b8] mb-4">Formation</h2>
          <div
      ...
      <aside class="pt-[56px] space-y-7">
        ...
        <div v-if="hasSkills">
          <h2 class="text-[7pt] uppercase tracking-[0.2em] font-bold text-[#94a3b8] mb-3">Compétences</h2>
          <ul class="space-y-2">
            <li
              v-for="(skill, i) in snapshot.skills"
              :key="i"
              class="flex items-start justify-between gap-2 text-[8.5pt] text-[#334155]"
            >
              <div class="flex items-start gap-2 min-w-0 flex-1">
                <span class="w-1 h-1 rounded-full shrink-0 mt-1.5" :style="{ backgroundColor: accent }" />
                <span class="break-words leading-tight">{{ skill.name }}</span>
              </div>
              <span v-if="formatSkillLevel(skill.level)" class="text-[7.5pt] text-[#94a3b8] shrink-0 mt-0.5">
        ...
        <div v-if="hasLanguages">
          <div class="h-px bg-[#f1f5f9] mb-7" />
          <h2 class="text-[7pt] uppercase tracking-[0.2em] font-bold text-[#94a3b8] mb-3">Langues</h2>
          <ul class="space-y-2">
            <li
              v-for="(lang, i) in snapshot.languages"
              :key="i"
              class="text-[8.5pt] text-[#334155] flex justify-between items-center"
            >
              <span>{{ lang.name }}</span>
        ...
        <div v-if="hasInterests">
          <div class="h-px bg-[#f1f5f9] mb-7" />
          <h2 class="text-[7pt] uppercase tracking-[0.2em] font-bold text-[#94a3b8] mb-3">Centres d'intérêt</h2>
          <ul class="space-y-2">
            <li
